  ...
  return selectedUser ? (
    <div className=' h-full overflow-auto relative backdrop-blur-lg' >
      <div className='flex items-center py-3 gap-3 mx-4 border-b border-stone-500'>
        ...
        <img onClick={() => selectedUser(null)} src={assets.arrow_icon} alt='Arrow_Icon' className=' md:hidden max-w-7' />
        <img src={assets.help_icon} alt='ArrowIcon' className=' max-md:hidden max-w-5' />
      </div>
      {/* Chat Area  */}
      <div className=' flex flex-col h-[calc(100%-120px)] overflow-y-auto p-3 pb-6'>
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId != authUser._id && 'flex-row-reverse'}`}>
            {
              msg.image ? (<img src={msg.image} alt='' className=' max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />) : (<p className={`p-2 max-w[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId == authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>)
            }
