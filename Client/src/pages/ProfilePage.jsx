    ...
    <div className=' min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className=' flex flex-col gap-5 p-10 flex-1'>
          ...
          <input onChange={(e) => setName(e.target.value)} value={name} type='text' placeholder='Your Name' className='p-2 border border-gray-500 rounded-md focus:outline:-none focus:ring-2 focus:ring-voilet-500' required />

          <textarea onChange={(e) => setBio(e.target.value)} value={bio} rows={4} placeholder='Write Profile Bio' className='p-2 border border-gray-500 rounded-md focus:outline:-none focus:ring-2 focus:ring-voilet-500' required></textarea>
